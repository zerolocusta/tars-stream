...
fn put_head(buf: &mut BytesMut, tag: u8, tars_type: TarsTypeMark) {
    if tag < 15 {
...
pub trait EncodeTo {
    fn encode_into_bytes(&self, tag: u8, buf: &mut BytesMut);
}
...
mod tests {
    ...
    fn test_put_head() {
        ...
        put_head(&mut buf, 19, EnInt8);
        put_head(&mut buf, 0, EnInt16);
    ...
    fn test_encode_i8() {
        let mut buf = BytesMut::new();
        let i0: i8 = -127;
        i0.encode_into_bytes(0, &mut buf);
        assert_eq!(&buf[..], &b"\x00\x81"[..]);

        let i1: i8 = 127;
        i1.encode_into_bytes(14, &mut buf);
        assert_eq!(&buf[..], &b"\x00\x81\xe0\x7f"[..]);

        let i2: i8 = -1;
        i2.encode_into_bytes(255, &mut buf);
    ...
    fn test_encode_u8() {
        let mut buf = BytesMut::new();
        let u0: u8 = 127;
        u0.encode_into_bytes(0, &mut buf);
        assert_eq!(&buf[..], &b"\x00\x7f"[..]);

        let u1: u8 = 255;
        u1.encode_into_bytes(14, &mut buf);
        assert_eq!(&buf[..], &b"\x00\x7f\xe0\xff"[..]);

        let u2: u8 = 0;
        u2.encode_into_bytes(255, &mut buf);
    ...
    fn test_encode_i16() {
        ...
        let mut buf = BytesMut::new();
        let i1: i16 = -127;
        i1.encode_into_bytes(15, &mut buf);
        assert_eq!(&buf[..], &b"\xf0\x0f\x81"[..]);

        let mut buf = BytesMut::new();
        let i2: i16 = 32767;
        i2.encode_into_bytes(19, &mut buf);
