...
struct DecoderPositionGuard<'a> {
    decoder: &'a TarsDecoder,
    before_pos: usize,
}

impl<'a> DecoderPositionGuard<'a> {
    fn rollback(&self) {
        self.decoder.pos.store(self.before_pos, Ordering::Relaxed);
    }
}

impl<'a> From<&'a TarsDecoder> for DecoderPositionGuard<'a> {
    fn from(decoder: &'a TarsDecoder) -> DecoderPositionGuard {
        DecoderPositionGuard {
            decoder,
            before_pos: decoder.pos.load(Ordering::Relaxed),
        }
    }
}

impl<'a> Drop for DecoderPositionGuard<'a> {
    fn drop(&mut self) {
...
impl TarsDecoder {
    ...
    //     T: DecodeFromTars,
    // {
    //     let decoder = TarsDecoder::from(buf);
    //     let head = decoder.take_head()?;
    //     decoder.read(head.tars_type)
    // }

    fn remaining(&self) -> usize {
        self.buf.len() - self.pos.load(Ordering::Relaxed)
    }
    ...
    fn advance(&self, cnt: usize) -> Result<(), DecodeErr> {
        if self.remaining() < cnt {
            Err(DecodeErr::NoEnoughDataErr)
        } else {
            self.pos.fetch_add(cnt, Ordering::Relaxed);
            Ok(())
        }
    }
    ...
    fn take_then_advance(&self, size: usize) -> Result<Bytes, DecodeErr> {
        if self.remaining() < size {
            ...
            self.pos.fetch_add(size, Ordering::Relaxed);
            Ok(b)
        }
    }

    fn skip_to_tag(&self, tag: u8) -> Result<Head, DecodeErr> {
    ...
    fn take_head(&self) -> Result<Head, DecodeErr> {
        if self.remaining() < 1 {
            Err(DecodeErr::NoEnoughDataErr)
        } else {
    ...
    fn skip_field(&self, tars_type: TarsTypeMark) -> Result<(), DecodeErr> {
        match tars_type {
    ...
    fn skip_string1_field(&self) -> Result<(), DecodeErr> {
        let mut buf = self.take_then_advance(1)?.into_buf();
        let size = buf.get_u8() as usize;
        self.advance(size)
    }

    fn skip_string4_field(&self) -> Result<(), DecodeErr> {
        let mut buf = self.take_then_advance(4)?.into_buf();
        let size = buf.get_u32_be() as usize;
        self.advance(size)
    }

    fn skip_map_field(&self) -> Result<(), DecodeErr> {
    ...
    fn skip_list_field(&self) -> Result<(), DecodeErr> {
        let ele_size = self.read_int32(0)? as usize;
        for _ in 0..ele_size {
            let head = self.take_head()?;
            self.skip_field(head.tars_type)?;
    ...
    fn skip_simple_list_field(&self) -> Result<(), DecodeErr> {
        let head = self.take_head()?; // consume header (list type)
        self.skip_field(head.tars_type)
    }

    fn skip_struct_field(&self) -> Result<(), DecodeErr> {
...
pub trait TarsDecodeSimpleTrait {
    fn read_int8(&self, tag: u8) -> Result<i8, DecodeErr>;
    fn read_boolean(&self, tag: u8) -> Result<bool, DecodeErr>;
    fn read_int16(&self, tag: u8) -> Result<i16, DecodeErr>;
    fn read_int32(&self, tag: u8) -> Result<i32, DecodeErr>;
    fn read_int64(&self, tag: u8) -> Result<i64, DecodeErr>;

    fn read_uint8(&self, tag: u8) -> Result<u8, DecodeErr>;
    fn read_uint16(&self, tag: u8) -> Result<u16, DecodeErr>;
    fn read_uint32(&self, tag: u8) -> Result<u32, DecodeErr>;

    fn read_float(&self, tag: u8) -> Result<f32, DecodeErr>;
    fn read_double(&self, tag: u8) -> Result<f64, DecodeErr>;
    fn read_string(&self, tag: u8) -> Result<String, DecodeErr>;

    fn read_bytes(&self, tag: u8) -> Result<Bytes, DecodeErr>;
...
where
    T: DecodeFromTars,
{
    fn read_list(&self, tag: u8) -> Result<Vec<T>, DecodeErr>;
}
...
where
    ...
    V: DecodeFromTars,
{
    fn read_map(&self, tag: u8) -> Result<BTreeMap<K, V>, DecodeErr>;
    fn read_map_optional(
        &self,
        tag: u8,
        default_value: Option<BTreeMap<K, V>>,
    ) -> Result<Option<BTreeMap<K, V>>, DecodeErr>;
}
...
where
    T: DecodeFromTars,
{
    fn read_struct(&self, tag: u8) -> Result<T, DecodeErr>;
    fn read_struct_optional(
        &self,
        tag: u8,
...
impl TarsDecodeSimpleTrait for TarsDecoder {
    // 对于 require field，不需要回滚 pos，查找失败将会返回 TagNotFound, 表示此二进制流非法
    fn read_int8(&self, tag: u8) -> Result<i8, DecodeErr> {
    ...
    fn read_int8_optional(
        ...
        // 对于 optional field，查找失败时需要回滚 pos
        let _g = DecoderPositionGuard::from(self);
        match self.read_int8(tag) {
            Err(e) => match e {
                DecodeErr::TagNotFoundErr => Ok(default_value),
                _ => Err(e),
            },
            Ok(v) => Ok(Some(v)),
        }
    ...
    fn read_boolean_optional(
        ...
        self.read_int8_optional(tag, None)
            .map(|opt| opt.map_or(default_value, |i| Some(i != 0)))
    }
...
where
    T: DecodeFromTars,
{
    default fn read_list(&self, tag: u8) -> Result<Vec<T>, DecodeErr> {
        let head = self.skip_to_tag(tag)?;
        match head.tars_type {
            EnList => {
                let size = self.read_int32(0)? as usize;
...
impl TarsDecodeListTrait<i8> for TarsDecoder {
    fn read_list(&self, tag: u8) -> Result<Vec<i8>, DecodeErr> {
        self.read_bytes(tag)
...
impl TarsDecodeListTrait<bool> for TarsDecoder {
    fn read_list(&self, tag: u8) -> Result<Vec<bool>, DecodeErr> {
        self.read_bytes(tag)
            .map(|v| unsafe { mem::transmute(v.to_vec()) })
    }
}
    ...
    V: DecodeFromTars,
{
    ...
    // we can just match read_map()，unlike reading simple type, decode_from_tars() may also
    ...
    T: DecodeFromTars,
{
    ...
    // we can just match read_struct()，unlike reading simple type, decode_from_tars() may also
    // return TagNotFoundErr
    fn read_struct_optional(
        &self,
        tag: u8,
        ...
        match self.skip_to_tag(tag) {
            ...
            Ok(head) => match head.tars_type {
                EnStructBegin => Ok(Some(T::decode_from_tars(self, 0)?)),
                _ => Err(DecodeErr::MisMatchTarsTypeErr),
            },
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_decode_zero() {
        let de = TarsDecoder::from(&b"\x0c\x1c\x2c\x3c\x4c\x5c\xfc\xff\x9c\xac\xec"[..]);
        let o0 = de.read_int8_optional(128, None).unwrap();
        let o1 = de.read_int8_optional(128, Some(13)).unwrap();
    ...
    fn test_take_map() {
        ...
        let de = TarsDecoder::from(&b[..]);
        let map: BTreeMap<String, String> = de.read_map(4).unwrap();
        let value2 = map.get(&String::from(&"foo bar"[..])).unwrap();
        assert_eq!(value2, &String::from(&"hello world"[..]));

        let b2: [u8; 6] = [0x48, 0x02, 0, 0, 0, 0];
        let de2 = TarsDecoder::from(&b2[..]);
        let map2: BTreeMap<Vec<String>, BTreeMap<i32, String>> = de2.read_map(4).unwrap();
    ...
    fn test_take_int64() {
        ...
        let de2 = TarsDecoder::from(header_vec.as_slice());
        let i: i64 = de2.read_int64(255).unwrap();
    ...
    fn test_take_int32() {
        ...
        header_vec.extend_from_slice(&b);
        let de2 = TarsDecoder::from(header_vec.as_slice());
        let i: i32 = de2.read_int32(255).unwrap();
    ...
    fn test_decode_int16() {
        ...
        header_vec.extend_from_slice(&b);
        let de2 = TarsDecoder::from(header_vec.as_slice());
        let i: i16 = de2.read_int16(14).unwrap();
    ...
    fn test_decode_int8() {
        ...
        let de2 = TarsDecoder::from(header_vec.as_slice());
        let i: i8 = de2.read_int8(14).unwrap();
        assert_eq!(i, -1);

        let de2 = TarsDecoder::from(header_vec.as_slice());
        let i = de2.read_int8_optional(15, None).unwrap();
        let i2 = de2.read_int8_optional(14, None).unwrap();
        assert_eq!(i, None);
        assert_eq!(i2, Some(-1));
    }
    ...
    fn test_decode_string() {
        ...
        let de2 = TarsDecoder::from(&d2[..]);
        assert_eq!(
            de2.read_string(2).unwrap(),
            String::from(&"foo bar"[..])
        );
    }
    ...
    fn test_decode_bool() {
        ...
        let de = TarsDecoder::from(&d[..]);
        let b: bool = de.read_boolean(0).unwrap();
        let ob: Option<bool> = de.read_boolean_optional(2, None).unwrap();

        let b2: bool = de.read_boolean(1).unwrap();

        let ob2: Option<bool> = de.read_boolean_optional(3, Some(true)).unwrap();

        assert_eq!(b, false);
        assert_eq!(b2, true);
        assert_eq!(ob, None);
        assert_eq!(ob2, Some(true));
    }
